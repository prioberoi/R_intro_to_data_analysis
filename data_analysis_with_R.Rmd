...
### Moving away from statistical tests to statistical learning

Can we predict what the typical speed will be if given the advertised speed?
```{r}
fit <- lm(typicdown ~ maxaddown, data)
summary(fit)

ggplot(data, aes(x = typicdown, y = maxaddown)) +
  geom_point() +
  geom_smooth(method = "lm") +
  ggtitle("Scatter plot of advertised and typical download speeds with linear regression line")

residualdata <- data.frame(maxaddown = data$maxaddown, residual = resid(fit))
ggplot(residualdata, aes(x = maxaddown, y = residual)) +
